"""
Cálculo de polinomios de Adomian para una función no lineal N y para sistemas.
"""
...
class AdomianPolynomialsCalculator:
    ...
    @staticmethod
    def compute(N_expr: Expr, u_components: List[Expr], n: int, dep_var:
                Function) -> Expr:
        ...
        lam = sp.Symbol('l')
        u_series = sum(uk * lam**k for k, uk in enumerate(u_components[:n+1]))
    ...
    @staticmethod
    def compute_sequence(N_expr: Expr, u_components: List[Expr], max_n: int,
                         dep_var: Function) -> List[Expr]:
        return [AdomianPolynomialsCalculator.compute(N_expr, u_components,\
                                        n, dep_var) for n in range(max_n+1)]
    ...
    def compute_for_system(
        ...
        components_dict: diccionario {dep_var: [u0, u1, ..., un]} para
        cada variable.
        ...
        if not depends: return N_expr if n == 0 else sp.S(0)
        
        lam = sp.Symbol('l')
        subs_dict = {}
        for var in dep_vars:
            comps = components_dict.get(var, [])
            series = sum(comps[k] * lam**k for k in range(min(n+1, len(comps))))
