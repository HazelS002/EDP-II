...
def extract_initial_conditions(conditions: List[Condition], var: Symbol) -> Dict[int, Expr]:
    """
...
def build_homogeneous_solution(order: int, var: Symbol, point: Expr, init_vals: Dict[int, Expr]) -> Expr:
    """
    Construye la solución homogénea phi = sum_{k=0}^{order-1} C_k * (var - point)^k
    y determina las constantes C_k usando init_vals (diccionario orden->valor).
    """
...
def get_base_point(conditions: List[Condition], default=0) -> Union[Expr, Dict[Symbol, Expr]]:
    """Obtiene el punto base común de las condiciones iniciales."""
    for cond in conditions:
        if cond.is_initial:
            return cond.at_point
    return default


def apply_initial_conditions(expr: Expr, var: Symbol, conditions: List[Condition]) -> Expr:
    """
